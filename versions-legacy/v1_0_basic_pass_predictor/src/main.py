"""
Minimal satellite pass predictor using SGP4.

Behavior:
1. Load a TLE from a file
2. Propagate the orbit for the next 48 hours at 30-second intervals
3. Compute satellite position in TEME and convert to ECEF
4. Convert to topocentric for a fixed ground station
5. Detect passes where elevation > 10 degrees
6. Output pass start time, max elevation time, and end time as JSON

Notes:
- This implementation uses a simple GMST rotation (TEME->ECEF approx).
- For higher fidelity, consider full IAU2000 transformations.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal satellite pass predictor using SGP4")
    p.add_argument("--tle", type=str, default="data/tle.txt", help="Path to TLE text file")
...
def main():
    ...
    # Propagate and compute elevations
    elevations = []
    for dt in times:
